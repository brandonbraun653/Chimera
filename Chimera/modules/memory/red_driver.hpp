...
namespace Red
{
  ...
  /**
   *   RedFSPosix is a thinly veiled wrapper around the Reliance Edge Posix C file
   * system driver. The purpose is to allow the system to be reused without the
   * backend function calls being tied to a specific hardware driver. This
   * transforms the interaction model into more of an object oriented interface.
   *   Under the hood, the class is doing varying amounts of redirection to figure
   * out which device is actually using the FS.
   *
   *   @note   All documentation (with minor modification) is courtesy of Reliance
   * Edge.
   *
   *
   */
  ...
  public:
     ...
     *
     *   Prepares the Reliance Edge file system driver to be used.  Must be the
     * first Reliance Edge function to be invoked: no volumes can be mounted or
     * formatted until the driver has been initialized.
     *
     ...
     *
     *   @param[in]  device      An object that implements the BlockDevice class.
     * This is the actual hardware device where data will be stored.
     *   @param[in]  volumeName  The volume string associated with the device.
     * (Available volumes listed in redconf.c)
     ...
     *
     *   Tears down the Reliance Edge file system driver.  Cannot be used until
     * all Reliance Edge volumes are unmounted.  A subsequent call to init() will
     *   initialize the driver again.
     *
     *   If this function is called when the Reliance Edge driver is already
     *   uninitialized, it does nothing and returns success.
     *
     *   This function is not thread safe: attempting to uninitialize from
     * multiple threads could leave things in a bad state.
     *
     *   @return On success, zero is returned. On error, -1 is returned and
     * red_errno is set appropriately.
     */
     ...
     *
     *   Remove's the object that was registered in init(), but does not tear down
     * the actual file system driver.
     */
     ...
     *
     *   This function discards the in-memory state for the file system and marks
     * it as unmounted.  Subsequent attempts to access the volume will fail until
     ...
     *   thread to complete by acquiring the FS mutex.  The volume will be marked
     * as unmounted before the FS mutex is released, so subsequent FS threads will
